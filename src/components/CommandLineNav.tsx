...
export default function CommandLineNav() {
    ...
    return (
        <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-black/90 backdrop-blur-md border-b border-lime/20 py-2' : 'bg-transparent py-4'
            }`}>
            <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between font-mono text-sm">
                <div className="flex items-center gap-2 text-lime">
                    <span className="text-magenta">guest@portfolio</span>
                    <span className="text-lime-dim">:</span>
                    <span className="text-blue-400">~{pathname === '/' ? '' : pathname}</span>
